...
st.caption("News Flow · Screener investisseur · données live Yahoo Finance")
...
flux_news = {
    ...
    "Apple": "https://news.google.com/rss/search?q=Apple+stock+finance&hl=en-US&gl=US&ceid=US:en",

    "Nvidia": "https://news.google.com/rss/search?q=Nvidia+stock+finance&hl=en-US&gl=US&ceid=US:en",

    "ASML": "https://news.google.com/rss/search?q=ASML+stock+finance&hl=en-US&gl=US&ceid=US:en",

    "LVMH": "https://news.google.com/rss/search?q=LVMH+stock+finance&hl=en-US&gl=US&ceid=US:en"
...
def recuperer_news(selection, nombre_news):

    articles = []

    for nom in selection:

        if nom not in flux_news:

            continue

        feed = feedparser.parse(flux_news[nom])
...
articles = recuperer_news(selection, nombre_news)
